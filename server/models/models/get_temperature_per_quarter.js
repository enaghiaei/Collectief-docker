    ...
    */
    ...
    var i = 0;
       ...
       ////console.log(sql)
        ...
        if (rows4.length == 0) {
            [rows, fields] = await con.execute("SELECT sensor_serial,AVG(measure_value) AS av,MAX(measure_value) AS av_max,MIN(measure_value) AS av_min,'" + h + " to " + h_2 + "' AS range_title,measure_kind,measure_name FROM node_2204036" + num_t + "  WHERE " + " timestamp >= '" + x0 + "'" + " AND timestamp < '" + x1 + "' GROUP BY sensor_serial,measure_kind,measure_name");
           //console.log(con.sql);
