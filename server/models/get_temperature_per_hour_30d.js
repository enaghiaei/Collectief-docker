    ...
    */
    ...
    for (var i = 1; i > 0; i--) {
       ...
       ////console.log(sql)
        ...
        [rows, fields] = await con.execute("SELECT max(nph_avg) AS av,max(nph_max) AS av_max,max(nph_min) AS av_min,nph_from2 AS range_title,nph_name AS measure_name,cl_id,nph_kind AS measure_kind FROM node_per_4hour JOIN collectief_assignment ON collectief_assignment.sensor_id = node_per_4hour.sensor_serial  WHERE ca_deleted=0 AND nph_from2 >= '" + x0 + "'" + " AND nph_from2 <= '" + x1 + "' GROUP BY cl_id,nph_kind,nph_name,nph_from2");
